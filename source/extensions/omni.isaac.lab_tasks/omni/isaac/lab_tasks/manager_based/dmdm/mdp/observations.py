...
def joint_pos_rel(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=['FL_hip_joint', 'FR_hip_joint', 'RL_hip_joint', 'RR_hip_joint', 'FL_thigh_joint', 'FR_thigh_joint', 'RL_thigh_joint', 'RR_thigh_joint', 'FL_calf_joint', 'FR_calf_joint', 'RL_calf_joint', 'RR_calf_joint'])) -> torch.Tensor:
...
def joint_vel_rel(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=['FL_hip_joint', 'FR_hip_joint', 'RL_hip_joint', 'RR_hip_joint', 'FL_thigh_joint', 'FR_thigh_joint', 'RL_thigh_joint', 'RR_thigh_joint', 'FL_calf_joint', 'FR_calf_joint', 'RL_calf_joint', 'RR_calf_joint'])):
...
"""
Motors for rod control 
"""

# TODO
def rod_motor_position(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=["rod_motor_position"])):  #joint_ids=slice(13, 14)

    asset_cfg.resolve(env.scene)
    asset: Articulation = env.scene[asset_cfg.name]
    return asset.data.joint_pos[:, asset_cfg.joint_ids]

def base_motor_position(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot", joint_names=["base_motor_velocity"])):
    
    asset_cfg.resolve(env.scene)
    asset: Articulation = env.scene[asset_cfg.name]
    return asset.data.joint_pos[:, asset_cfg.joint_ids]
# def all_joints_names(env: ManagerBasedEnv, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")):
#     asset: Articulation = env.scene[asset_cfg.name]
#     return asset.data.joint_names[asset_cfg.joint_ids]
...
RayCaster sensor
'''
def height_scan(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg, offset: float = 0.5) -> torch.Tensor:   #0.5
...
def imu_orientation(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg = SceneEntityCfg("imu")) -> torch.Tensor:
...
def imu_position(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg = SceneEntityCfg("imu")) -> torch.Tensor:
    """Imu sensor position in the simulation world frame.

    Args:
        env: The environment.
        asset_cfg: The SceneEntity associated with an IMU sensor. Defaults to SceneEntityCfg("imu").
    Returns:
        The position (m) in the world frame. Shape is (num_envs, 3).
    """
    asset: Imu = env.scene.sensors[sensor_cfg.name]
    return asset.data.pos_w

def imu_lin_vel(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg = SceneEntityCfg("imu")) -> torch.Tensor:
    """Imu sensor linear velocity in the simulation world frame.
    
    Args:
        env: The environment.
        asset_cfg: The SceneEntity associated with an IMU sensor. Defaults to SceneEntityCfg("imu").
    Returns:
        The linear velocity (m/s) in the world frame. Shape is (num_envs, 3).
    """
    asset: Imu = env.scene.sensors[sensor_cfg.name]
    return asset.data.lin_vel_b

def imu_lin_acc(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg = SceneEntityCfg("imu")) -> torch.Tensor:
...
def imu_ang_vel(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg = SceneEntityCfg("imu")) -> torch.Tensor:
...
def imu_ang_acc(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg = SceneEntityCfg("imu")) -> torch.Tensor:
    """Imu sensor angular acceleration w.r.t. environment origin expressed in the sensor frame.
    ...
    Returns:
        The angular acceleration (rad/s^2) in the sensor frame. Shape is (num_envs, 3).
    """
    asset: Imu = env.scene.sensors[sensor_cfg.name]
    return asset.data.ang_acc_b

def feet_real_time_position(env: ManagerBasedEnv, feet_contact_sensor_cfg: SceneEntityCfg = SceneEntityCfg("contact_sensor")) -> torch.Tensor:
    """Foot sensor position in the simulation world frame.

    Args:
        env: The environment.
        asset_cfg: The SceneEntity associated with a foot sensor. Defaults to SceneEntityCfg("contact_sensor").

    Returns:
        The position (m) in the world frame. Shape is (num_envs, 3).
    """
    feet_contact_sensor: ContactSensor = env.scene.sensors[feet_contact_sensor_cfg.name]
    return feet_contact_sensor.data.pos_w



def contact_sensor_pos(env: ManagerBasedEnv, sensor_cfg: SceneEntityCfg) -> torch.Tensor:

    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    return contact_sensor.data.pos_w.squeeze()
